  ...
  return (
    <div className='text-center bg-orange-400 p-4 '>
      ...
      <nav className='flex flex-row p-4 font-semibold text-lg mx-4'>
        ...
        <Link to={'/about'} className='pr-8' >About Us</Link>
        <Link to={'/login'} className='pr-8'>Login</Link>
        <Link to={'/profile'} className='pr-8'>Profile</Link>
        
      </nav>
      <div>
        <label className='px-1 font-semibold'>Select language</label>
        <select className='border-black border p-2' value={lang} onChange={(e)=> setLang(e.target.value)}>
